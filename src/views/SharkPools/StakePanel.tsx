...
const StakePanel: React.FC<StakePanelProps> = ({ id, updateResult }) => {
    ...
    const renderButtons = () => {
        ...
        if (!pool.userInfo.paidUnlock) {
            return (<button onClick={handleUnlock} className="btn btn-disabled w-100" type="button">Unlock Pool</button>);
        }
        ...
        return (
            <div>
                <DisplayFlex>
                    <span style={{ paddingRight: '50px' }} className="total-earned text-shadow">{getFullDisplayBalance(new BigNumber(pool.userInfo.stakedAmount), pool.stakeToken.decimals, 4)}</span>
