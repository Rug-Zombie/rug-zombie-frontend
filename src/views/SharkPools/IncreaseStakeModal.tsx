...
const IncreaseStakeModal: React.FC<IncreaseStakeModalProps> = ({ id, updateResult, onDismiss }) => {
    ...
    if (bigStakeAmount.gt(tokenBalance)) {
        ...
        isDisabled = true
        stakingDetails = `Invalid Stake: Minimum ${getBalanceAmount(pool.poolInfo.minStake).toString()} ${pool.stakeToken.symbol}`
    } else if (bigStakeAmount.plus(pool.userInfo.stakedAmount).gt(pool.poolInfo.maxStake)) {
        isDisabled = true
        stakingDetails = `Invalid Stake: Maximum ${getBalanceAmount(pool.poolInfo.maxStake).toString()} ${pool.stakeToken.symbol}`
    }

    return(
        <Modal onDismiss={onDismiss} title={`Stake ${pool.stakeToken.symbol}`} headerBackground={theme.colors.gradients.cardHeader}>
