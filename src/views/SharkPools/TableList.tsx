...
import { getBep20Contract, getPancakePair } from '../../utils/contractHelpers'
...
const TableList: React.FC<TableListProps> = ({ id, openHandler }) => {
            ...
            lpTokenContract.methods.getReserves().call()
              .then(reservesRes => {
                  lpTokenContract.methods.totalSupply().call()
                    .then(totalSupplyRes => {
                      const reserve = getBalanceAmount(new BigNumber(pool.bnbLpTokenIndex === 0 ? reservesRes._reserve0.toString() : reservesRes._reserve1.toString()))
    ...
    }, [bnbPrice, pool.bnbLpTokenIndex, pool.lpPool, pool.stakeToken.address])
                ...
                <tr>
                    ...
                    {isDesktop
                        ? <td className='td-width-25'>
                            <DisplayFlex>
                                <span
                                    className='total-earned'>{numeral(getBalanceAmount(pool.poolInfo.totalStaked).times(stakeTokenPrice)).format('($ 0.00 a)')}</span>
