...
const CurrentGraves: React.FC<CurrentGravesProps> = (props) => {
  const { grave } = props
  const link = `https://app.euler.tools/token/${
    grave.name === 'RugZombie Common' ? '0x50ba8BF9E34f0F83F96a340387d1d3888BA4B3b5' : grave.rug.address[56]
  }`
  return (
    <Container>
      <Flex style={{ justifyContent: 'center' }}>
        <LinkExternal href={link} target="_blank">
          <Text color="green">{grave.name}</Text>
        </LinkExternal>
      </Flex>
    </Container>
  )
