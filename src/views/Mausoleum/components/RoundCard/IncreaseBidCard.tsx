...
import { CardBody, LinkExternal, PlayCircleOutlineIcon, Button, Text, Flex } from '@rug-zombie-libs/uikit'
...
export const CurrentBid: React.FC<CurrentBidProps> = ({ totalAmount, id, ...props }) => {
  ...
  return (
    <Flex alignItems="center" justifyContent="space-between" {...props}>
      ...
      <Text bold>
        {getBalanceAmount(totalAmount).toString()} {v3 ? 'BNB' : 'BT'}
      </Text>
    </Flex>
  )
}
      ...
      <Card className="mbCardStyle">
        ...
        <CardBody p="16px">
          <RoundResultBox>
            ...
            {paidUnlockFee || v3 ? (
              allowance.gt(BIG_ZERO) || v3 ? (
                <>
                  <CurrentBid totalAmount={amount} id={id} mb="8px" />
                  <Button variant="success" width="100%" onClick={increaseBid} mb="4px">
                    {t('Increase BID')}
                  </Button>
                  <Button variant="danger" width="100%" onClick={decreaseBid} mb="4px">
                    {t('Decrease BID')}
                  </Button>
                  ...
                  <Flex alignItems="center" justifyContent="center" className="indetails-title" paddingTop="10px">
                    <LinkExternal onClick={() => handleSetPosition(BetPosition.BULL)} fontSize="14px">
                      {t('Enter custom bid')}
                    </LinkExternal>
                  </Flex>
                </>
