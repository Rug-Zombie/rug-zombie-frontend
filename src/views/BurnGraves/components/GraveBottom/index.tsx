...
const GraveBottom: React.FC<RugInDetailsProps> = ({ pid, zombieUsdPrice, account }) => {
  const {
    subtitle,
    rug,
    pcsVersion,
    ...
    <div key={pid} className="rug-indetails">
      <div className="direction-column imageColumn">
        <div className="sc-iwajpm dcRUtg">
          {type === 'image' ? <img src={path} alt="NFT" className="sc-cxNHIi bjMxQn" /> : <Video path={path} />}
        </div>
      </div>
      <div className="direction-column">
        ...
        <span className="indetails-title">
          ...
          <span className="indetails-value">
            {numeral(getBalanceAmount(poolInfo.totalStakingTokenStaked).times(zombieUsdPrice)).format('($ 0.00 a)')}
          </span>
        </span>
        <span className="indetails-title">
