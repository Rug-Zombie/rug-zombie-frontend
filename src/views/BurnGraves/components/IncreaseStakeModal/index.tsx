...
const IncreaseStakeModal:React.FC<IncreaseStakeModalProps> = ({ id, updateResult, onDismiss }) => {
  ...
  const handleChangePercent = (sliderPercent: number) => {
    ...
    if (grave.userInfo.stakedAmount.toString() === '0') {
      percentageOfStakingMax = tokenBalance.minus(grave.poolInfo.minimumStake).dividedBy(100).multipliedBy(sliderPercent)
  ...
  if (bigStakeAmount.gt(tokenBalance)) {
    ...
    isDisabled = true
    stakingDetails = `Invalid Stake: Minimum ${getBalanceAmount(grave.poolInfo.minimumStake).toString()} ${grave.stakingToken.symbol}`
  }
  
  return(
    <Modal onDismiss={onDismiss} title={`Stake ${grave.stakingToken.symbol}`} headerBackground={theme.colors.gradients.cardHeader}>
      ...
      {tokenBalance.toString() === '0'
        ? <Button mt='8px' as='a' external
                  href={`${BASE_EXCHANGE_URL}/swap?outputCurrency=${getAddress(grave.stakingToken.address)}`} variant='secondary'>
