...
const TableList: React.FC<TableListProps> = ({ id, handler }) => {
                ...
                <tr>
                            ...
                            <div className="into-two-td">
                                <div className="info-1">
                                    <div className="info-icon">
                                        ...
                                        <img src={`images/tokens/${pool.stakeToken.symbol}.png`} alt="rugicon" className="icon" />
                                    </div>
                                    <div>
                                        <div className="titel">{pool.name}</div>
                                        <div className="small-lable">
                                            <div className="small-titel">{pool.stakeToken.symbol}</div>
                                            {pool.isNew ? <div className='con-info' style={{marginLeft: "4px"}}>NEW!</div> : null}
                                        </div>
                                    </div>
                                </div>
                            </div>
                    ...
                    {isDesktop 
                        ? <td className='td-width-25'>
                            <DisplayFlex>
                                <span
                                    className='total-earned'>{numeral(getBalanceAmount(pool.totalStaked).times(pool.usdPrice)).format('($ 0.00 a)')}</span>
