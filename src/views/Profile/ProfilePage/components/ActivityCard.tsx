    ...
    switch (type) {
      case UserActivityType.DrFDeposit:
        if(tombPids().includes(pid)) {
          return <ActivityText>Staked {numeral(getBalanceNumber(amount)).format('(0.00 a)')} LP in {getDrFPoolName(pid)} tomb</ActivityText>
        }
        return <ActivityText>Staked {numeral(getBalanceNumber(amount)).format('(0.00 a)')} ZMBE in {getDrFPoolName(pid)} grave</ActivityText>
      ...
      case UserActivityType.DrFWithdraw:
        if(tombPids().includes(pid)) {
          return <ActivityText>Unstaked {numeral(getBalanceNumber(amountWithdrawn)).format('(0.00 a)')} LP from {getDrFPoolName(pid)} tomb</ActivityText>
        }
        return <ActivityText>Withdrew {numeral(getBalanceNumber(amountWithdrawn)).format('(0.00 a)')} ZMBE from {getDrFPoolName(pid)} grave</ActivityText>
      case UserActivityType.DrFWithdrawEarly:
        if(tombPids().includes(pid)) {
          return <ActivityText>Unstaked {numeral(getBalanceNumber(amountWithdrawn)).format('(0.00 a)')} LP Early from {getDrFPoolName(pid)} tomb</ActivityText>
        }
        return <ActivityText>Unstaked {numeral(getBalanceNumber(amountWithdrawn)).format('(0.00 a)')} ZMBE Early from {getDrFPoolName(pid)} grave</ActivityText>
