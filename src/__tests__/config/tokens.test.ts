...
const symbolsExcludedFromAllTests = new Set(['BNB', 'Zombie (No Relation)', ''])

const symbolsExcludedFromSymbolTest = new Set(['FINS-BNB LP', 'JAWS-BNB LP'])
...
const tokensToTest = omitBy(tokens, (token) => symbolsExcludedFromAllTests.has(token.symbol))
...
describe('Config tokens', () => {
  ...
  beforeAll(async () => {
    const tokenAddressesToTest = Object.values(tokensToTest).map(({ address }) => address[56])
    const results: Array<Array<string | number>> = await multicall(
      erc20ABI,
      tokenAddressesToTest.flatMap((address) => [
        { address, name: 'symbol' },
  ...
  it.each(map(tokensToTest, (token, key) => [key, token]))(
    ...
    (key, token: Token) => {
      ...
      if (!keysExcludedFromKeyTest.has(key)) {
        expect(toComparableSymbol(key)).toBe(toComparableSymbol(token.symbol))
      }

      if (!symbolsExcludedFromSymbolTest.has(token.symbol)) {
        expect(toComparableSymbol(token.symbol.toLowerCase())).toBe(toComparableSymbol(symbol.toLowerCase()))
      }
