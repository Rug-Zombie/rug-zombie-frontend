...
describe('Config graves', () => {
  it.each(gravesToTest.map((grave) => grave.pid[56]))('Grave #%d has an unique pid', (pid) => {
    const duplicates = graves.filter((grave) => pid === grave.pid[56])
    expect(duplicates).toHaveLength(1)
  })
  it.each(gravesToTest.map((grave) => [grave.pid[56], grave.rug]))(
    'Grave #%d has an unique rugged token address',
    (_, ruggedToken) => {
      const duplicates = graves.filter((f) => ruggedToken === f.rug)

      // @ts-ignore
      const pidsPermittedToHaveSameRuggedToken =
        'address' in ruggedToken && expectedDuplicateRugGravesByRuggedTokenAddress.get(ruggedToken.address[56])
      if (!pidsPermittedToHaveSameRuggedToken) {
        expect(duplicates).toHaveLength(1)
      } else {
        duplicates.map(({ pid }) => pid[56]).forEach((pid) => expect(pidsPermittedToHaveSameRuggedToken).toContain(pid))
      }
    },
  )
  it.each(gravesToTest)('Grave %p has the correct rugged token addresses', async (grave) => {
    const { rug } = grave
