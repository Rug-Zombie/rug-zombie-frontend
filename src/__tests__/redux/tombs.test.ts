...
describe('Config tombs', () => {
  ...
  it.each(tombsToTest)('tomb %p has the correct token addresses', async (tomb) => {
    const tokenAddress = tomb.token.address[56]
    const quoteTokenAddress = tomb.quoteToken.address[56]
  ...
  it.each(tombsToTest)('tomb %p has non 0 tokens amount', async (tomb) => {
    const tokenContract = getBep20Contract(tomb.token.address[56])
    const quoteTokenContract = getBep20Contract(tomb.quoteToken.address[56])
