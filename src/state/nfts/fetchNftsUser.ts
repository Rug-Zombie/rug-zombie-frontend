...
const fetchNftsUser = async (account: string, nftsToFetch: Nft[]): Promise<NftIdAndUserInfo[]> => {
  ...
  const resultBatches: NftIdAndUserInfo[][] = await Promise.all(chunk(nftsToFetch, 32)
      .map((nftBatch) => contract.methods.massCheckOwnership(
          account, (nftBatch.map(nft => getAddress(nft.address)))).call()
          .then((ownedIdResults) => nftBatch.map((nft, index) => ({
            id: nft.id,
            userInfo: {
